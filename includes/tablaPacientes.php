...
$pacientes=mysqli_query($con,"SELECT * FROM pacientes WHERE status=1 ORDER BY id_paciente DESC");
 ?>
                            ...
                            <?php while($p=mysqli_fetch_array($pacientes)){?>
                                <tr>
                                    ...
                                    <td>
                                         ...
                                         <button class="btn btn-sm" title="Editar Datos" style="background: none;border: none;margin-top: -10px;" onclick="editar('<?php echo $p['id_paciente'] ?>','<?php echo $p['dni']; ?>','<?php echo $p['nombres']; ?>','<?php echo $p['apellidos']; ?>','<?php echo $p['telefono']; ?>','<?php echo $p['direccion']; ?>','<?php echo $p['email']; ?>')">
                                            <img src="assets/img/editar.png" width="30px" alt="">
                                        </button>

                                        <button class="btn btn-sm" title="Eliminar" onclick="eliminar('<?php echo $p['id_paciente']; ?>')" style="background: none;border: none;margin-top: -10px;">
